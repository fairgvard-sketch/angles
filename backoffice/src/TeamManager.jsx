...
function StaffRow({ member, onEdit, onChangePin, editable }) {
  return (
    <div className={`menu-row${member.is_active ? '' : ' is-off'}`}>
      <span className="menu-name">
        {member.name}
        {!member.is_active && <small> · inactive</small>}
      </span>
      <span className="menu-price">{ROLE_LABELS[member.role] || member.role}</span>
      {editable ? (
...
function StaffEditor({ member, locations, canAssignOwner, canDelete, onClose, onSaved }) {
  ...
  const roles = canAssignOwner ? ROLES : ROLES.filter((r) => r !== 'owner')
  ...
  async function save() {
    ...
    try {
      if (isNew) {
        await createStaff({ name, role, pin, locationId })
      } else {
        await updateStaff(member.id, { name: name.trim(), role, is_active: active })
