...
/**
 * Что важнее сказать о госте одним словом.
 *
 * Сначала то, что требует действия («уходит», «пропал», «не приходит по
 * броням»), затем ценность, и только потом стадия знакомства. Внутри
 * стадии выигрывает более частное: у кого семь визитов, тот «Regular», а
 * не «Returning» — обе метки верны, но вторая ничего не добавляет.
 */
const SEGMENT_RANK = [
  'at_risk', 'lost', 'repeat_no_show',
  'vip', 'regular', 'returning', 'new', 'upcoming',
]

/**
 * Основная подпись строки — самая содержательная из меток гостя.
 *
 * Сервер (155) отдаёт набор от общего к частному, поэтому `segments[0]`
 * — наименее говорящая метка: гость с семью визитами приезжал оттуда
 * «Returning». В узкой колонке помещается одна, и это должна быть та, за
 * которой владелец сюда пришёл; остальные видны в карточке.
 */
export function primarySegment(segments) {
  if (!Array.isArray(segments) || segments.length === 0) return null
  return SEGMENT_RANK.find((key) => segments.includes(key)) ?? segments[0]
}
