...
## 14.09.2026 — выпуск B4/B5: карточка товара и серверная проверка состава

Дата / исполнитель: 14.09.2026, Claude (после передачи работы).
Задача плана: B — гостевой заказ. Открытая карточка настройки при обновлении
каталога (B4) и серверная граница состава заявки (B5, миграция 171).
Коммит anglesite: эта запись (документация).
Коммит kassa: `aefb585` (main, запушен), база `fa73c06`.
Среда: production `qgmnxrgtlpyqglwqmsej`, схема 170 → **171**.
Продукт, роль: публичное меню и POS-бандл, гость; SQL-лаборатория — отдельный
контейнер `supabase/postgres:17.6.1.141`, синтетическая база `kassa_synth_b5`
(общий локальный стек не запускался, `supabase db reset` не выполнялся).

Сценарий:

1. Открытая карточка товара во время обновления каталога: цена, название,
   исчезнувшие размер и добавка, новая обязательная группа, исчезнувший товар.
2. Приём заявки с негодным составом: `min_select`, `max_select`, товар с
   размерами без `variant_id`.

Ожидалось: устаревший состав и цена в корзину и в заявку не попадают; выбор
за гостя не делается; инварианты 116 (org-скоуп, capability, смена, часы,
table-token, лимит частоты, идемпотентность, предел 10 модификаторов,
серверный расчёт цены) сохраняются.

Получено:

- Воспроизведение до правки — PASS (дефект подтверждён). Клиент:
  `PublicOrderConfigSheet.test.tsx` 1/7 (зелёным был только положительный
  контроль). Сервер на схеме 170 принимал все четыре негодных состава, в том
  числе товар с размерами по базовой цене 800 вместо 1400.
- После правки — PASS. Клиент 7/7; pgTAP `online_order_composition.test.sql`
  17/17, на прежнем теле 116 того же набора 12/17 (красные ровно новые
  проверки).
- Весь pgTAP проекта на синтетической базе с 171: 86 файлов, 1751
  утверждение, 0 красных.
- `npm run lint`, `test:all` (768 Vitest + 42 bundle + 11 ops), `build`,
  `check:bundle` (62.0/240.0 modern, 130.4/310.0 legacy KiB gzip),
  `check:schema` (v171) — PASS.
- Копия production снята до миграции (`manifest.json`, status complete,
  три файла) — PASS.
- `supabase db push --linked`: применена одна миграция
  `171_online_order_composition.sql`; удалённый журнал показывает 171 — PASS.
- CI на точном коммите `aefb585` — success:
  https://github.com/fairgvard-sketch/pos/actions/runs/34823071101
- Публикации: POS и Menu отдают новый i18n-чанк с текстами `pubItemGone` и
  `pubErrComposition` (проверено содержимое бандла, не только код ответа) —
  PASS. Публичный smoke: `/setup`, `/sell`, `/`, `/order/:id` — 200, ассеты
  JS/CSS — 200 с корректным MIME.

Доказательство: `/private/tmp/angle-b4-config-CzzV4h/artifacts/`
(`CONFIG-SHEET-report.md`, `ORDER-COMPOSITION-report.md`, патчи, логи baseline
и after, pgTAP-логи).

Ограничения и повторная проверка (NOT RUN):

- Живой гостевой заказ через Edge Function и HTTP не выполнялся: запись
  синтетики в production в постпроверку не входит.
- Физический T2, Safari/iPhone, старая вкладка с прежним service worker —
  NOT RUN.
- Штатный `supabase start && supabase db reset && supabase test db` не
  запускался; pgTAP прогонялся в воссозданной вручную эквивалентной среде
  (схемы `auth`/`storage` дополнены, см. отчёт B5).
- Серверная проверка состава на production подтверждена только фактом
  применения миграции, не живым вызовом.
